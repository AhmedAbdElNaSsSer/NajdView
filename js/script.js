...
function citiesInCountries() {
    ...
    if(bodyJob) {
        ...
        console.log('country: ', country.getAttribute('data-value'));
        form.country.addEventListener('change', () => {
            const keyExists = country.value in arabCountries;
            if (keyExists) {
                city.classList.remove('dis')
                city.parentElement.classList.remove('notall')
                let cities = arabCountries[country.value]
                console.log('cities: ', cities);
                city.innerHTML=`<option value="empty" selected disabled>المدينة *</option>`
                cities.map((singleCity) => {
                    city.innerHTML += `<option value="${singleCity}">${singleCity}</option>`
                })
                
            } else {
            alert(country.value + " لا توجد مدينة تسمي");
            }
        })
    }
} citiesInCountries()
